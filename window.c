...
void upload_image(t_game *so_long)
{
    ...
    so_long->imgs.exit = mlx_xpm_file_to_image(so_long->mlx, "textures/exit.xpm", &x,
                                               &y);
    if (!so_long->imgs.wall || !so_long->imgs.empty || !so_long->imgs.player || !so_long->imgs.collectable || !so_long->imgs.exit)
    {
...
static void put_imag(t_game *so_long, int x, int y)
{
    ...
    if (so_long->map[y][x] == '1')
        mlx_put_image_to_window(so_long->mlx, so_long->mlx_win, so_long->imgs.wall, x * 64, y * 64);
    else if (so_long->map[y][x] == 'E')
        mlx_put_image_to_window(so_long->mlx, so_long->mlx_win, so_long->imgs.exit, x * 64, y * 64);
        ...
        mlx_put_image_to_window(so_long->mlx, so_long->mlx_win, so_long->imgs.player,
                                x * 64, y * 64);
    if (so_long->map[y][x] == 'E' && so_long->player.x == so_long->exit.x && so_long->exit.y == so_long->player.y)
        mlx_put_image_to_window(so_long->mlx, so_long->mlx_win, so_long->imgs.player,
                                x * 64, y * 64);
}
